...
def rsvm_train(train_x, train_y, val_x, val_y, val_g, model_path, params):
    # get the cost and gamma
    c_range = params['rsvm_c']
    g_range = params['rsvm_g']
    metric_dict = {}
    for c in c_range:
        for g in g_range:
    ...
    top_n = params['top_n']['rsvm']
    # save the best params to csv file
    save_params(top_n, results_order, params['param_keys']['rsvm'], model_path)
    prob_dict = {}
...
def lsvm_train(train_x, train_y, val_x, val_y, val_g, model_path, params):
    # get the cost and gamma
    c_range = params['lsvm_c']
    metric_dict = {}
    for c in c_range:
        print('** C: %d  **' % c)
        clf = SVC(kernel='linear', C=2 ** c, probability=True)
        clf.fit(train_x, train_y)
        val_prob = clf.predict_proba(val_x)[:, 1]
        # metric: auc, aupr, ndcg@k, roc@k
        metric_df = evaluation(params['metrics'], val_y, val_prob, val_g)
    ...
    results_order = sorted(metric_dict.items(), key=lambda x: x[1], reverse=True)
    # select top_n best model
    top_n = params['top_n']['lsvm']
    # save the best params to csv file
    save_params(top_n, results_order, params['param_keys']['lsvm'], model_path)
    prob_dict = {}
    for i in range(top_n):
        hp = results_order[i][0]
        clf = SVC(kernel='linear', C=2 ** hp[0], probability=True)
        clf.fit(train_x, train_y)
        joblib.dump(clf, model_path + 'model[top_' + str(i+1) + '].pkl')
        val_prob = clf.predict_proba(val_x)[:, 1]
        prob_dict['top_' + str(i+1)] = val_prob
